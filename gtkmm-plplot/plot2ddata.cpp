...
Plot2DData::Plot2DData(
  ...
  x(_x), y(_y), color(_color), line_style(_line_style), shown(true) {
  if (x.size() != y.size()) {
...
Plot2DData::Plot2DData(
  ...
  Color _color,
  LineStyle _line_style) :
  Plot2DData(indgen(_y.size()), _y, _color, _line_style) {}
...
Plot2DData::Plot2DData(const Plot2DData &_data) :
  Plot2DData(_data.x, _data.y, _data.color, _data.line_style) {
    shown = _data.shown;
  }
