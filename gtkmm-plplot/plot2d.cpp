...
void Plot2D::set_axis_time_format_x(Glib::ustring _time_format) {
  if (!_time_format.empty()) {
    if (get_axis_logarithmic_x()) {
      throw Exception("Gtkmm::Plplot::Plot2D::set_axis_time_format_x -> X-axis cannot be time and logarithmic");
    }
  }
...
Glib::ustring  Plot2D::get_axis_time_format_x() {
  return time_format;
}

void Plot2D::config_time_x(double _scale, const Glib::DateTime& _time) {
  time_scale = _scale;
  time_start = _time;
}
